...
function playGame() {
    ...
    endGame.addEventListener('click', (event) => {
        ...
        if (finalLevel === true) {
            if (ans === true) {
                // Won by answering level 15 correctly — currentLevel is already 15 here
                saveLevel(playerName, currentLevel);
            } else {
                // Lost — currentLevel is the level they failed on, so the last cleared one is currentLevel - 1
